    ...
    try {
      ...
      if (!results) {
        throw new NotFoundError(
          `Stock tickers not found: ${tickers.join(', ')}`,
        );
      }
  ...
  private mapToHistoricalData(
    ...
    for (const quote of quotes) {
      ...
      const dataPoint: HistoricalData = {
        ...
        close:
          quote.close === undefined ? 0 : Math.round(quote.close * 100) / 100,
      ...
      if (fields && fields.length > 0) {
        ...
        fields.forEach((field) => {
          if (
            field in dataPoint &&
            dataPoint[field as keyof HistoricalData] !== undefined
          ) {
  ...
  private validateHistoricalDataDates(fromDate: string, toDate: string): void {
    ...
    if (!isValid(fromDateObj)) {
      throw new ValidationError(
        `Invalid fromDate: ${fromDate}. Use YYYY-MM-DD format.`,
      );
    }
    if (!isValid(toDateObj)) {
      throw new ValidationError(
        `Invalid toDate: ${toDate}. Use YYYY-MM-DD format.`,
      );
    }
